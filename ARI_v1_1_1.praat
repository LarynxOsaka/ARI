########################################################################
form ARI ver.1_1_1 based on SFEEDS ver.1_1_0 (cs＋sv)
...
comment ▼Draw the results as a figure
  boolean draw_Figure 1
  boolean save_Figure 1

comment ▼Save the detailed results of quantification
sentence Save_directory
sentence Title_of_table Table_of_ARI
...
# Preserve every object that existed before ARI started.
# All Remove commands in this script are routed through ariSafeRemove,
# which never removes these pre-existing object IDs.
select all
...
clearinfo
indexline$ = "FileName" + tab$ + "CS_FileName" + tab$ + "SV_FileName" + tab$ + "ARI" + tab$ + "ABI" + tab$ + "chaoticNoise_Power_of_all" 
indexline$ = indexline$ + tab$ + "superSubharmonics_Power_of_all" + tab$ + "one_3rdSubharmonics_Power_of_all" + tab$
indexline$ = indexline$ + "one_halfSubharmonics_Power_of_all" + tab$ + "superSubharmonics_of_all" + tab$ + "one_3rdSubharmonics_of_all"
indexline$ = indexline$ + tab$ + "one_halfSubharmonics_of_all" + tab$ + "finalhfno6000" + tab$ + "finalhnrd" + tab$ + "finalH1H2" + tab$
indexline$ = indexline$ + "finalhnrd_ABI60" + tab$ + "finalH1H2_ABI60" + tab$ + "cpps" + tab$ + "jitterLocal" + tab$ + "shimmerLocal" + tab$ + "shimmerLocaldB" + tab$ + "psd" + tab$
indexline$ = indexline$ + "gneMaximum" + tab$ + "slope" + tab$ + "tilt" + tab$ + "hnr" + tab$ + "Praatversion"

# Write one TSV header at the start of the run.
result_file$ = save_directory$ + "/" + title_of_table$ + ".tsv"
writeFileLine: result_file$, indexline$

# Keep the Info window for on-screen progress/results only.
echo 'indexline$'

############################################################
############################################################
#Concatenate CS+SV to simple
for sub from 1 to number_of_patients
#■Select CS Sample
select left_Sound_No. + (sub-1)
name1$ = selected$("Sound")
namecs$ = left$(name1$, 16)
#Resample the audio to 44100 Hz
sampleRate = Get sampling frequency
if sampleRate <> 44100
    # Resample creates and selects the new Sound. Keep that exact object selected;
    # do not re-select it by a generated name.
    Resample: 44100, 50
    Rename: name1$
endif
#Extract 1 ch if the audio sample has more than 1 channel.
ch = Get number of channels
if ch <> 1
    Extract one channel: 1
endif
Rename: name1$
#Keep the exact CS working object by ID.
ari_cs_work_id = selected("Sound")

#■Select SV Sample
select right_Sound_No. + (sub-1)
name2$ = selected$("Sound")
namesv$ = left$(name2$, 16)
#Resample the audio to 44100 Hz
sampleRate = Get sampling frequency
if sampleRate <> 44100
    # Resample creates and selects the new Sound. Keep that exact object selected;
...
#■Create the final SV working object after CS preprocessing.
#  > 3 s: use the central 3 seconds (same rule as v1.1.0).
# <= 3 s: copy the whole SV so that the working-object order is still CS -> SV.
...
if namesv$ <> namecs$
    ...
    exitScript: "Different patients of CS/SV sample. SampleID (first 16 characters) must match."
endif

#The SV working object is always created after the CS working object.
#Praat concatenates selected Sounds in Objects-list order, therefore this is CS -> SV.
...
resultline$ = namecs$ + tab$ + name1$ + tab$ + name2$
